# -*- coding: utf-8 -*-
"""
------- Main for Figure_3 OMP MSE-Loss, oracle weights, learned weights and relative ℓ2-error boxplots
...
core_data_name = f'N_{N}_m_{m}_s_{s}_K_{K}_noise_{noise_std}_size_{n_train + n_val}_batch_{batch_size}'
core_name = core_data_name + f'_tau_{tau}_lr_{learning_rate}'
### OMP data
data_name = 'OMPL_data_' + core_data_name + '.pt'
data_name = os.path.join(os.path.join(script_dir, 'data', 'Fig_3'), data_name)
# dataset = OMPLData(m, N, s, N_levels, s_levels, measure, noise_std, n_train, n_val, batch_size)
...
### MLP OMP
model = MLP_soft_OMP(L = num_layers, D = dataset.A, num_inputs = input_size,\
                num_outputs = output_size, s = s, tau = tau, n_OMP_layers = 0,\
                    n_soft_OMP_layers = s)
### Loss and optimizer
...
loss_OMP = vec_MSELoss(dataset.X_hat[:, n_train:].T, dataset.X[:, n_train:].T)
w_oracle = torch.zeros(N, 1)
w_oracle[dataset.idx_total[:K], :] = 1
X_womp_oracle = torch.zeros(N, n_val)
for i in range(n_val):
    X_womp_oracle[:, i] = OMP(dataset.A, dataset.Y[:, n_train + i].unsqueeze(1), max_it = s, W = w_oracle).real.squeeze(1)
loss_womp_oracle = vec_MSELoss(X_womp_oracle.T, dataset.X[:, n_train:].T)
...
print("Model loss (before training):", loss_model_before, "\n OMP loss", loss_OMP,\
      "\n OMP_median:", torch.median(torch.norm(dataset.X_hat[:, n_train:].T - dataset.X[:, n_train:].T, dim = 1)**2),\
          "\n WOMP-oracle loss", loss_womp_oracle, "\n WOMP-soft oracle loss", loss_wsoft_oracle)

#%% #### Main files
### Training loop
def main_train():
    checkpoint_name = 'OMP_checkpoint_' + core_name
    config = train_config(
    ...
    train_loss_name = 'OMPNet_train_loss_' + core_name + '.pt'
...
def best_checkpoint_finder():
    ...
    for i, epoch in enumerate(checkpoint_epoch_range):
        checkpoint_name = 'OMP_checkpoint_' + core_name + f'_epoch_{epoch}.pt'
        ...
        W_evolution[:, i] = checkpoint['model_dict']['layer_soft_OMP.W']
    ...
    checkpoint_name = 'OMP_checkpoint_' + core_name + f'_epoch_{epoch_best_model}.pt'
    checkpoint_name = os.path.join(script_dir, 'data', 'Fig_3', 'checkpoints', checkpoint_name)
    checkpoint = torch.load(checkpoint_name)
    epoch_best = checkpoint_epoch_range[i_best_model]
    
    best_checkpoint_name = 'OMP_best_checkpoint_' + core_name + '.pt'
...
def main_fig_gen():
    train_loss_name = 'OMPNet_train_loss_' + core_name + '.pt'
    train_loss_name = os.path.join(script_dir, 'data', 'Fig_3', train_loss_name)
    # train_loss_name = os.path.join(script_dir, 'data', 'Fig_3', 'OMPNet_train_loss_N_256_m_22_s_10_noise_0.001_batch_64_ntrain_1024_nval_512_lr_0.01.pt')
    ...
    w_learned = checkpoint['model_dict']['layer_soft_OMP.W']
    
    model.load_state_dict(checkpoint['model_dict'])
    X_model = model((dataset.Y[:, n_train:]).T).real.detach()
    
    X_womp_learned = torch.zeros(N, n_val)
    for i in range(n_val):
        X_womp_learned[:, i] = OMP(dataset.A, dataset.Y[:, n_train + i].unsqueeze(1), max_it = s, W = w_learned.unsqueeze(1)).real.squeeze(1)
    loss_womp_learned = vec_MSELoss(X_womp_learned.real.T, dataset.X[:, n_train:].T)
    ...
    plt.figure()
    plt.plot(train_iter_v, train_loss_v, linewidth = 2, label = "OMP-Net (training)")
    plt.plot(val_iter_v, val_loss_v, linewidth = 2, label = "OMP-Net (validation)")
    # plt.plot(val_iter_v, loss_womp_oracle*torch.ones(len(val_iter_v)), linewidth = 2, label = "WOMP (oracle)")
    # plt.plot(val_iter_v, loss_womp_learned*torch.ones(len(val_iter_v)), linewidth = 2, linestyle = '--', label = "WOMP (learned)")
    plt.plot(val_iter_v, loss_best_model*torch.ones(len(val_iter_v)), linewidth = 2, label = "OMP-Net")
    plt.plot(val_iter_v, loss_OMP*torch.ones(len(val_iter_v)), linewidth = 2, label = "OMP")
    plt.yscale('log')
    plt.xlim([0, epoch_best + 0.1])
    plt.ylabel("MSE-Loss")
    plt.xlabel("epoch")
    plt.legend(loc = 'upper right')
    fig_name = 'OMP_fig_' + core_name + '.png'
    fig_name = os.path.join(script_dir, 'figs', 'Fig_3', fig_name)
    fig = plt.gcf()
    fig.savefig(fig_name, bbox_inches='tight', dpi=300)
    print("OMP loss:", loss_OMP, "\n WOMP-oracle loss:", loss_womp_oracle, "\n WOMP-soft oracle loss:", loss_wsoft_oracle,\
          "\n WOMP_learned loss:", loss_womp_learned, "\n Model loss (before training):", loss_model_before, "\n Model loss (after training):", loss_best_model)
    ...
    fig_name = 'OMP_weights_' + core_name + '.png'
    fig_name = os.path.join(script_dir, 'figs', 'Fig_3', fig_name)
    fig.savefig(fig_name, bbox_inches='tight', dpi=300)
    
    plt.figure(figsize = (6.4, 3.2))
    err_omp = torch.norm((dataset.X_hat[:, n_train:].real.T - dataset.X[:, n_train:].T), dim = 1)\
        /torch.norm(dataset.X[:, n_train:].T, dim = 1)
    err_womp_oracle = torch.norm((X_womp_oracle.real.T - dataset.X[:, n_train:].T), dim = 1)\
        /torch.norm(dataset.X[:, n_train:].T, dim = 1)
    err_womp_learned = torch.norm((X_womp_learned.real.T - dataset.X[:, n_train:].T), dim = 1)\
        /torch.norm(dataset.X[:, n_train:].T, dim = 1)
    err_model = torch.norm((X_model.real - dataset.X[:, n_train:].T), dim = 1)\
        /torch.norm(dataset.X[:, n_train:].T, dim = 1)
    # plt.boxplot(torch.cat((err_omp.unsqueeze(0), err_womp_learned.unsqueeze(0), err_womp_oracle.unsqueeze(0), err_model.unsqueeze(0))))
    # plt.xticks([1, 2, 3, 4], ['OMP', 'WOMP (learned)', 'WOMP (oracle)', 'OMP-Net'])
    plt.boxplot(torch.cat((err_omp.unsqueeze(0), err_model.unsqueeze(0))), vert = False)
    plt.yticks([1, 2], ['OMP', 'OMP-Net'])
    plt.xscale('log')
    plt.xlabel('Relative $\ell^2$-error')
    fig_name = 'OMP_boxplot_' + core_name + '.png'
    fig_name = os.path.join(script_dir, 'figs', 'Fig_3', fig_name)
    fig = plt.gcf()
    fig.savefig(fig_name, bbox_inches='tight', dpi=300)
