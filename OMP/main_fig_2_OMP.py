...
def main_fig2_data(key_val):
    if key_val == 0:
        err_omp = torch.zeros(n_repeat, n_std)
        err_soft_omp = torch.zeros(n_repeat, n_std, n_tau)
...
def main_fig2_plot(key_val):
    ...
    if key_val == 0:
        ...
        mu_omp = torch.log10(err_omp).mean(dim = 0)
        mu_soft_omp= torch.log10(err_soft_omp).mean(dim = 0)
        std_omp = torch.log10(err_omp).std(dim = 0)
        std_soft_omp = torch.log10(err_soft_omp).std(dim = 0)
        for i in range(n_std):
            ...
            y_omp = 10**(mu_omp[i]*torch.ones(tau_range.size()))
            plt.plot(tau_range, y_omp, color = color_profile[i][0], alpha = 1, label = 'OMP ($ \sigma $ = ' + "$10^{{{}}}$)".format(int(b)))
            y_omp_up = 10**((mu_omp[i] + std_omp[i])*torch.ones(tau_range.size()))
            plt.plot(tau_range, y_omp_up, color = color_profile[i][0], alpha = 0.2)
            y_omp_down = 10**((mu_omp[i] - std_omp[i])*torch.ones(tau_range.size()))
